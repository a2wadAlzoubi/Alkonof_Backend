...
public class DelayBookingConsumer(ILogger<DelayBookingConsumer> logger, IApplicationDbContext context) : IConsumer<CustomerAnswerAssignedEvent>
{
    public async Task Consume(ConsumeContext<CustomerAnswerAssignedEvent> consumeContext)
    {
        if (consumeContext.Message.Decision == Decision.Delay)
        {
