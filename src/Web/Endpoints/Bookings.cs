...
using Alkonof_Backend.Application.Modulers.Bookings.Book.Queries.GetBookingByCustomerId;
using Alkonof_Backend.Application.Modulers.Bookings.Book.Queries.GetBookingById;
using Alkonof_Backend.Application.Modulers.Bookings.Book.Queries.GetBookingByResponsibleId;
...
using Alkonof_Backend.Application.Modulers.Bookings.Book.Queries.GetExpiredBooking;
using Alkonof_Backend.Application.Modulers.Bookings.Book.Queries.GetPendingBooking;
...
public class Bookings : IEndpointGroup
{
    public static void Map(RouteGroupBuilder group)
    {
        ...
        bookingsGroup.MapGet("/customer/{customerId:guid}", GetBookingByCustomerId).RequireAuthorization();
        bookingsGroup.MapGet("/responsible/{responsibleId:guid}", GetBookingByResponsibleId).RequireAuthorization();
        bookingsGroup.MapGet("/", GetBookings).RequireAuthorization();
        #endregion
        ...
        #region Booking Query by State Endpoints
        bookingsGroup.MapGet("/pending", GetPendingBooking).RequireAuthorization();
        bookingsGroup.MapGet("/expired", GetExpiredBooking).RequireAuthorization();
        bookingsGroup.MapGet("/confirmed", GetConfirmedBooking).RequireAuthorization();
        #endregion
    }
    ...
    [EndpointSummary("Get bookings by customer ID")]
    public static async Task<Ok<List<BookingDto>>> GetBookingByCustomerId(ISender sender, Guid customerId)
    {
        var query = new GetBookingByCustomerIdQuery(customerId);
        var bookings = await sender.Send(query);
        return TypedResults.Ok(bookings);
    }

    [EndpointSummary("Get bookings by responsible ID")]
    public static async Task<Ok<List<BookingDto>>> GetBookingByResponsibleId(ISender sender, Guid responsibleId)
    {
        var query = new GetBookingByResponsibleIdQuery(responsibleId);
        var bookings = await sender.Send(query);
        return TypedResults.Ok(bookings);
    }

    [EndpointSummary("Get all bookings")]
    public static async Task<Ok<List<BookingDto>>> GetBookings(ISender sender)
    {
    ...
    #region Booking Query by State Handlers
    [EndpointSummary("Get pending bookings")]
    public static async Task<Ok<List<BookingDto>>> GetPendingBooking(ISender sender)
    {
        var query = new GetPendingBookingQuery();
        var bookings = await sender.Send(query);
        return TypedResults.Ok(bookings);
    }

    [EndpointSummary("Get expired bookings")]
    public static async Task<Ok<List<BookingDto>>> GetExpiredBooking(ISender sender)
    {
        var query = new GetExpiredBookingQuery();
        var bookings = await sender.Send(query);
        return TypedResults.Ok(bookings);
    }

    [EndpointSummary("Get confirmed bookings")]
    public static async Task<Ok<List<BookingDto>>> GetConfirmedBooking(ISender sender)
    {
        var query = new GetConfirmedBookingQuery();
        var bookings = await sender.Send(query);
        return TypedResults.Ok(bookings);
    }
    #endregion
